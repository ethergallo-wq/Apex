  ...
  return (
    <div data-home-layout="v2" style={{ height: '100%', display: 'flex', flexDirection: 'column', position: 'relative', background: isLightTheme ? LIGHT_APP_BG : 'radial-gradient(circle at 82% -8%, rgba(184,77,58,.16), transparent 34%), radial-gradient(circle at 12% 88%, rgba(144,216,74,.10), transparent 28%), linear-gradient(180deg,#101216,#0B0D10)', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 28px', WebkitOverflowScrolling: 'touch' }}>
        ...
        {dailyDiscovery?.animal && (
          <div style={{ borderRadius: 22, border: '1px solid rgba(157,211,255,.26)', background: isLightTheme ? 'rgba(255,255,255,.82)' : 'linear-gradient(135deg, rgba(157,211,255,.08), rgba(18,20,23,.92))', padding: 14, marginBottom: 16, boxShadow: '0 14px 32px rgba(0,0,0,.20)' }}>
            <div style={{ color: '#9DD3FF', fontSize: 10.5, fontWeight: 1000, letterSpacing: .9, textTransform: 'uppercase', marginBottom: 10 }}>Atlante · Scoperta del giorno</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              >
                <img
                  ...
                  alt=""
                  style={{ width: '100%', height: '100%', objectFit: 'cover', filter: dailyDiscovery.documented ? 'sepia(.72) saturate(.55) brightness(.94)' : 'brightness(0) saturate(0)', opacity: dailyDiscovery.documented ? 1 : .85 }}
                />
              </button>
              <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {dailyDiscovery.documented ? (
                <div style={{ flexShrink: 0, borderRadius: 14, padding: '9px 12px', background: 'rgba(157,211,255,.12)', border: '1px solid rgba(157,211,255,.34)', color: '#9DD3FF', fontSize: 11.5, fontWeight: 1000 }}>📖 Fatto</div>
              ) : (
                <button
                  type="button"
