...
// invece di mostrare il body nero, mostra un messaggio + bottone reload.
class AnimaldexErrorBoundary extends Component {
  ...
  componentDidCatch(error, info) {
    try {
      console.error('[Animaldex] Errore runtime:', error, info);
    } catch {}
  }
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div style={{ minHeight:'100dvh', maxWidth:480, margin:'0 auto', background:'#111113', color:'white', padding:'40px 24px', fontFamily:"'Sora',-apple-system,BlinkMacSystemFont,sans-serif", display:'flex', flexDirection:'column', justifyContent:'center', boxSizing:'border-box' }}>
          ...
          <p style={{ margin:'0 0 18px', color:'rgba(255,255,255,.66)', fontSize:13.5, lineHeight:1.5 }}>{msg}</p>
...
// che rispetta i colori canonici (bronzo / argento / oro / viola).
...
function ScaleComparison({ animal, full=false }) {
  ...
  // Box visivo disponibile: largo a sufficienza per stare vicini umano + animale.
  const boxWidthPx = full ? 320 : 96;
  const boxHeightPx = full ? 200 : 64;
  // Scala unica sia per l'umano (riferimento) sia per l'animale.
  // pxPerCm calcolato sul totale: (ref + spazio + animale) deve stare nel box.
  const animalCm = Math.max(1, lengthCm || ref.cm * 0.6);
  const totalCm = ref.cm + animalCm;
  // Margine minimo per non far toccare le silhouette: ~6% della larghezza del box
  const usableWidthPx = boxWidthPx * (full ? 0.92 : 0.96);
  const pxPerCm = usableWidthPx / totalCm;
  // Limite di altezza: l'umano (più alto) non deve uscire dal box.
  const heightCapPxPerCm = (boxHeightPx * (full ? 0.92 : 0.96)) / ref.cm;
  const finalPxPerCm = Math.min(pxPerCm, heightCapPxPerCm);
  // L'umano è sempre dimensionato per altezza (full body, busto, mano).
  const referenceVisibleHeight = ref.cm * finalPxPerCm;
  // L'animale è dimensionato per LARGHEZZA (la lunghezza corporea
  // corrisponde alla larghezza dell'immagine in profilo). Questo evita
  // che immagini in posa verticale facciano apparire l'animale più grande
    ...
    <div style={{ width:'100%', minHeight:full?220:60, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center' }}>
      ...
      {full && (
        <div style={{ color:'rgba(255,255,255,.58)', fontSize:12, fontWeight:800, marginTop:4, textAlign:'center', lineHeight:1.45 }}>
          {ref.label} · rapporto calcolato sulla parte visibile delle PNG. La larghezza dell'animale corrisponde alla lunghezza corporea reale ricavata da <strong style={{ color:'rgba(255,255,255,.76)', fontWeight:900 }}>ln</strong> (lunghezza naso-coda per i quadrupedi, lunghezza totale per pesci/squamati).
        </div>
      )}
    </div>
  );
        ...
        {(() => {
          ...
          return (
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:18 }}>
              <button onClick={()=>setMetricModal('peso')} style={metricBoxStyle}>
                <div style={metricVisualStyle}>
                  <div style={{ width:'100%', maxWidth:'116px' }}><GaugeSVG wt_str={a.wt} /></div>
                </div>
                <div style={{ ...metricLabelStyle, color:'white' }}>{a.wt}</div>
              </button>

              <button onClick={()=>setMetricModal('dimensioni')} style={metricBoxStyle}>
                <div style={metricVisualStyle}>
                  <ScaleComparison animal={a} />
                </div>
                <div style={metricLabelStyle}>{a.ln}</div>
              </button>

              <button onClick={()=>setMetricModal('trofico')} style={metricBoxStyle}>
                <div style={metricVisualStyle}>
                  <TrophicPyramid trophic={a.trophic} compact />
                </div>
                <div style={metricLabelStyle}>{activePyramidLevel.label}</div>
              </button>
            </div>
          );
        })()}
...
function MainMenu({ onOpen, onBack, onLogout, tutorialFocus=null, statusMap = {}, visitedCountries = [], earnedBadgeIds = [], userProfile, user, onOpenGridStatus, onOpenRegions, onQuickSeen, onOpenPhoto }) {
  ...
  // Source of truth unica: tutti i numeri vengono da progress.* — nessun ricalcolo separato.
  const searchedCount = progress.searchedCount;
  const seenCount = progress.seenCount;
  const capturedCount = progress.capturedCount;
  return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', overflow:'hidden', background:'radial-gradient(circle at 50% -10%, rgba(184,77,58,0.16), transparent 36%), linear-gradient(180deg, #11151B 0%, #0E1014 42%, #080A0D 100%)' }}>
      <PageHeader title="Mission Control" onBack={onBack} />
      <div style={{ flex:1, overflowY:'auto', padding:'12px 0 30px' }}>
        {/* Profile Pass */}
        <div className="profile-pass" style={{ marginTop:8 }}>
          <div style={{ display:'flex', alignItems:'center', gap:14 }}>
            <div className="profile-avatar" style={{ display:'flex', alignItems:'center', justifyContent:'center', color:'#D8D2C4', fontSize:32 }}>🧭</div>
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ marginTop:6, fontSize:13, fontWeight:800, color:'var(--text-muted)' }}>{progress.xp} / {nextLevelXP} XP</div>
            </div>
            <div style={{ color:'#F0A840', fontWeight:1000, fontSize:20 }}>🔥 3</div>
          </div>
        </div>
        ...
        <div className="main-mission-card">
          <div style={{ position:'relative', zIndex:2 }}>
            ...
            <button onClick={mission.action} className="mission-cta" style={{ fontFamily:'inherit', cursor:'pointer' }}>{mission.cta}</button>
          </div>
        </div>
        ...
        <div className="quick-actions">
          {[
            ['🔭','Ricercati',`${searchedCount} ${searchedCount === 1 ? 'specie' : 'specie'}`,()=>onOpenGridStatus?.(['ricercato'])],
        ...
        {/* Progress Card */}
        <button onClick={()=>onOpenGridStatus?.(['ricercato','avvistato','catturato'])} className="progress-card" style={{ cursor:'pointer' }}>
          {(() => {
            const unlockedCount = searchedCount + seenCount + capturedCount;
            const searchedPct = Math.max(0, Math.min(100, (unlockedCount / totalAnimals) * 100));
            const seenPct = Math.max(0, Math.min(100, (seenCount / Math.max(1, unlockedCount)) * 100));
            const capturedPct = Math.max(0, Math.min(100, (capturedCount / Math.max(1, unlockedCount)) * 100));
            const Row = ({ label, valueText, pct, fillClass }) => (
            ...
            return <>
              ...
              <div className="progress-subtitle">Il tuo avanzamento globale</div>
              <Row label="🔭 Ricercati" valueText={`${unlockedCount} / ${totalAnimals}`} pct={searchedPct} fillClass="progress-fill-searched" />
              <Row label="Avvistati"   valueText={`${seenCount} / ${Math.max(1,unlockedCount)}`} pct={seenPct} fillClass="progress-fill-seen" />
              <Row label="Catturati"   valueText={`${capturedCount} / ${Math.max(1,unlockedCount)}`} pct={capturedPct} fillClass="progress-fill-captured" />
            </>;
          })()}
        </button>

        {!!progress.nearlyCompletedBadges.length && <div style={{ margin:'22px 16px 0' }}>
        ...
        <div style={{ color:'rgba(255,255,255,.52)', fontSize:11, fontWeight:1000, textTransform:'uppercase', margin:'22px 16px 8px 18px' }}>Navigazione</div>
...
@keyframes ecosystemStress { 0%,100%{ transform:translate(0,0); } 25%{ transform:translate(1.5px,-1px); } 50%{ transform:translate(-1.3px,1px); } 75%{ transform:translate(1px,1.2px); } }`;
  ...
  useEffect(()=>{
    let mounted = true;
    // Pulisci eventuale hash OAuth (#access_token=...) dall'URL dopo che Supabase
    // ha già letto la sessione: evita che il redirect Google su Vercel lasci
    // l'URL "sporco" e generi stati di rendering anomali.
    ...
    // Failsafe: se qualcosa blocca il primo getSession (rete lenta, popup, ecc.),
    // non lasciamo l'utente in eterno su "Caricamento sessione...".
  ...
  // Failsafe profilo: se Supabase non risponde entro qualche secondo,
  // applichiamo il fallback locale così la UI non rimane mai bloccata
  // sullo schermo "Apertura Animaldex..." (nero) in produzione.
  useEffect(() => {
    if (!user?.id) return;
...
const renderDetailOverlay = () => enriched ? <div style={{ position:'absolute', inset:0, zIndex:80, background:'#1C1C1E' }}><Detail a={enriched} onBack={()=>setSel(null)} onStatusChange={handleStatusChange} onJumpToClass={jumpToClassFromDetail} onOpenComparator={openComparator} onOpenLifeWeb={openLifeWeb} onOpenPhoto={openPhotoRecognition} visitedCountries={visitedCountries} statusMap={statusMap} tutorialStep={tutorialStep} captureStamp={tutorialStamp} onTutorialAbilityClick={handleTutorialAbilityClick}/></div> : null;
  ...
  if (!userProfile) {
    // Niente più side-effects durante il render. Il fallback è gestito
    // dall'effetto qui sopra (failsafe profilo). Mostriamo solo il loader.
    return (
      <div style={{ height:'var(--animaldex-app-height, 100dvh)', maxWidth:480, margin:'0 auto', background:'#111113', color:'white', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Sora',-apple-system,BlinkMacSystemFont,sans-serif" }}>
