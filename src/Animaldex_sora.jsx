...
const STATUS_GRADIENTS = {
  misterioso:'linear-gradient(135deg,rgba(66,67,73,.58),rgba(28,29,33,.92))',
  ricercato:'linear-gradient(135deg,rgba(216,210,196,.30),rgba(50,50,54,.86))',
  avvistato:'linear-gradient(135deg,rgba(200,121,85,.46),rgba(76,43,33,.90))',
  catturato:'linear-gradient(135deg,rgba(184,77,58,.92),rgba(116,44,34,.96))',
};
const STATUS_FILTER_GRADIENTS = {
  misterioso:'linear-gradient(135deg,rgba(66,67,73,.40),rgba(32,33,38,.92))',
  ricercato:'linear-gradient(135deg,rgba(245,241,234,.24),rgba(58,58,62,.88))',
  avvistato:'linear-gradient(135deg,rgba(200,121,85,.42),rgba(72,45,34,.92))',
  catturato:'linear-gradient(135deg,rgba(184,77,58,.86),rgba(120,46,36,.96))',
};
...
function getAnimalCountryCodes(animal) {
  ...
  const geoIsoValues = geoIso && typeof geoIso === 'object'
    ? [...toArraySafe(geoIso.primary), ...toArraySafe(geoIso.iucn), ...toArraySafe(geoIso.observed), ...toArraySafe(geoIso.geoson_range)]
    : toArraySafe(geoIso);
  return normalizeIsoList([
    ...toArraySafe(animal?.distribution?.countries_present),
    ...geoIsoValues,
...
function useImagePixelBounds(src) {
  ...
  useEffect(() => {
    let alive = true;
    if (!src || typeof window === 'undefined') { setBounds(null); return; }
    const img = new Image();
    ...
    img.onload = () => {
      ...
      try {
        ...
        setBounds(found ? { naturalWidth:w, naturalHeight:h, x:minX, y:minY, width:maxX-minX+1, height:maxY-minY+1 } : { naturalWidth:w, naturalHeight:h, x:0, y:0, width:w, height:h });
      } catch {
        setBounds({ naturalWidth:w, naturalHeight:h, x:0, y:0, width:w, height:h });
      }
    };
    img.onerror = () => alive && setBounds(null);
    img.src = src;
    return () => { alive = false; };
  }, [src]);
  return bounds;
...
function normalizeSupabaseAnimal(row, userAnimal) {
  ...
  const geoIso = geo?.iso;
  const geoIsoValues = geoIso && typeof geoIso === 'object'
    ? [...toArraySafe(geoIso.primary), ...toArraySafe(geoIso.iucn), ...toArraySafe(geoIso.observed), ...toArraySafe(geoIso.geoson_range)]
    : toArraySafe(geoIso);
  const countries = normalizeIsoList([
    ...geoIsoValues,
    ...toArraySafe(raw?.distribution?.countries_present),
    ...toArraySafe(raw?.countries_present),
    ...toArraySafe(raw?.iso),
  ]);
...
function mergeRemoteWithLocalBioregions(remoteList = []) {
  return (remoteList || []).map(remote => {
    ...
    const remoteCountries = getAnimalCountryCodes(remote);
    const localCountries = getAnimalCountryCodes(local);
    const bestCountries = localCountries.length > remoteCountries.length ? localCountries : normalizeIsoList([...remoteCountries, ...localCountries]);
    const enrichedRemote = {
      ...
      distribution: {
        ...(local.distribution || {}),
        ...(remote.distribution || {}),
        countries_present: bestCountries,
      },
    };
...
async function removeUserDestination(userId, iso) {
  const cleanIso = String(iso || '').toUpperCase().trim();
  if (!userId || !cleanIso) return false;
  const { error } = await supabase
    .from('user_destinations')
...
#animaldex-app-root .animaldex-scroll-area {
  min-height: 0;
...
const TERRITORY_CAMERA_PRESETS = {
  america:{ center:[-88,18], zoom:1.55 },
  eurasia:{ center:[72,44], zoom:1.55 },
  africa:{ center:[20,2], zoom:1.8 },
  'oceania-australasia':{ center:[139,-25], zoom:2.05 },
  antartide:{ center:[15,-72], zoom:1.15 },
  'nord-america-boreale':{ center:[-100,62], zoom:2.2 },
  'nord-america-temperato':{ center:[-103,38], zoom:2.3 },
  'america-centrale-e-caraibi':{ center:[-78,17], zoom:2.8 },
  ...
  'africa-centrale-e-orientale':{ center:[25,1], zoom:2.35 },
  'africa-meridionale':{ center:[24,-24], zoom:2.45 },
  australasia:{ center:[136,-8], zoom:2.65 },
  oceania:{ center:[150,-23], zoom:2.2 },
};
    ...
    <div style={{ height:'100%', minHeight:0, maxHeight:'100%', background:isLightTheme?LIGHT_APP_BG:'radial-gradient(circle at 80% -8%, rgba(240,168,64,.34), transparent 34%), radial-gradient(circle at 10% 28%, rgba(184,77,58,.24), transparent 38%), radial-gradient(circle at 92% 72%, rgba(200,121,85,.20), transparent 36%), linear-gradient(180deg,#2A1208 0%,#1B100B 44%,#100B09 100%)', position:'relative', overflow:'hidden', overscrollBehavior:'none' }}>
      <div data-animaldex-bar="true" data-animaldex-grid-top="true" style={{ position:'absolute', top:0, left:0, right:0, height:gridTopChromeHeight, display:'flex', alignItems:'center', justifyContent:'space-between', gap:10, padding:isPhone?'calc(env(safe-area-inset-top, 0px) + 10px) 14px 8px':'calc(env(safe-area-inset-top, 0px) + 10px) 12px 10px', boxSizing:'border-box', marginTop:0, borderTop:'none', borderBottom:'none', background:ANIMALDEX_ORANGE_GRADIENT, borderRadius:'0 0 16px 16px', boxShadow:'0 8px 20px rgba(184,77,58,.18)', zIndex:30, overflow:'hidden', transform:'translateZ(0)' }}>
        {onBackToOrigin ? (
          ...
          <button onClick={onHome} aria-label="Home" style={gridControlStyle}>
...
const PHOTO_ID_TIPS = [
  'Avvicinati e metti l\'animale al centro.',
  'Usa luce naturale, evita controluce.',
  'Tieni la fotocamera ferma e a fuoco.',
  'Evita rami, vetri o ombre sul soggetto.',
];

function buildPhotoRecognitionMeta({ gps, file, uploadedPhoto, preview, aiResult, match }) {
  return {
    confidence: match?.confidence ?? null,
    gps,
    file,
    image_url: uploadedPhoto.publicUrl || preview,
    public_url: uploadedPhoto.publicUrl || '',
    storage_path: uploadedPhoto.storagePath || '',
    ai_status: aiResult?.identification_status || '',
    ai_summary: aiResult?.summary || '',
    ai_probability: match?.probability ?? null,
  };
}

function RecognitionAlternativeRow({ match, selected, onSelect, onRequestAdd, requestBusy }) {
  const inDex = !!match?.animal;
  const name = match.animal?.com || match.common_name || 'Specie non nel Dex';
  const sci = match.animal?.sci || match.scientific_name || '';
  const pct = Math.round(match.probability || 0);
  return (
    <div style={{ borderRadius:16, border:`1px solid ${selected ? 'rgba(144,216,74,.55)' : 'rgba(255,255,255,.10)'}`, background:selected ? 'rgba(144,216,74,.10)' : 'rgba(255,255,255,.045)', padding:10, marginBottom:8 }}>
      <button type="button" onClick={() => onSelect?.(match)} style={{ width:'100%', border:'none', background:'transparent', color:'white', display:'flex', alignItems:'center', gap:12, padding:0, cursor:'pointer', fontFamily:'inherit', textAlign:'left' }}>
        {inDex
          ? <AnimalImg a={match.animal} size={52} fontSize={24} overrideStatus="avvistato" />
          : <div style={{ width:52, height:52, borderRadius:14, background:'rgba(255,255,255,.07)', display:'grid', placeItems:'center', fontSize:22, flexShrink:0 }}>?</div>}
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, minWidth:0 }}>
            <div style={{ fontWeight:1000, fontSize:13, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{name}</div>
            <div style={{ color:'#90D84A', fontWeight:1000, fontSize:12, flexShrink:0 }}>{pct}%</div>
          </div>
          {sci && <div style={{ color:'rgba(255,255,255,.52)', fontSize:11, marginTop:4, lineHeight:1.25 }}>{sci}</div>}
          {match.reasoning && <div style={{ color:'rgba(255,255,255,.42)', fontSize:10.5, marginTop:4, lineHeight:1.3 }}>{match.reasoning}</div>}
        </div>
      </button>
      {!inDex && (
        <button type="button" disabled={requestBusy} onClick={() => onRequestAdd?.(match)} style={{ marginTop:8, width:'100%', minHeight:36, borderRadius:12, border:'1px solid rgba(240,168,64,.32)', background:'rgba(240,168,64,.10)', color:'#FFD9A8', fontFamily:'inherit', fontSize:11.5, fontWeight:900, cursor:requestBusy ? 'default' : 'pointer' }}>
          {requestBusy ? 'Invio richiesta…' : 'Richiedi aggiunta al Dex'}
        </button>
      )}
    </div>
  );
...
function PhotoRecognitionModal({ animal, animals = [], user, onClose, onConfirm, onRequestAddAnimal }) {
  ...
  const submitAddRequest = async (match) => {
    ...
    setError('');
    try {
      const result = await onRequestAddAnimal?.({
        common_name: match.common_name || match.animal?.com || '',
        scientific_name: match.scientific_name || match.animal?.sci || '',
        ai_summary: aiResult?.summary || match.reasoning || '',
        ai_probability: match.probability ?? null,
        photo_url: uploadedPhoto.publicUrl || preview,
        photo_storage_path: uploadedPhoto.storagePath || '',
        gps,
      });
      if (result?.ok) {
        setRequestNotice(result.mode === 'mailto'
          ? 'Si aprirà la mail per inviare la richiesta al team Apex.'
          : 'Richiesta inviata. La valuteremo e ti aggiorneremo quando la specie sarà nel Dex.');
      } else {
        setError('Non sono riuscito a inviare la richiesta. Riprova più tardi.');
      }
    } catch (err) {
  ...
  const aiStatus = aiResult?.identification_status || '';
  const activeMatch = selectedMatch || (aiStatus === 'certain' ? matches[0] : null);
  const activeInDex = !!activeMatch?.animal;
  const aiTone = aiStatus === 'certain' ? '#90D84A' : aiStatus === 'unusable' ? '#F0B24E' : '#9DD3FF';
  const aiTitle = aiStatus === 'certain'
    ? (activeInDex ? 'Identificazione certa' : 'Animale fuori dal Dex')
    : aiStatus === 'unusable'
  ...
  return (
    <div onClick={onClose} style={{ position:'fixed', inset:0, zIndex:260, background:'rgba(0,0,0,.82)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div onClick={e=>e.stopPropagation()} style={{ width:'100%', maxWidth:460, maxHeight:'92%', overflowY:'auto', borderRadius:28, background:'linear-gradient(180deg,#222226,#111113)', border:'1px solid rgba(255,255,255,.10)', boxShadow:'0 30px 80px rgba(0,0,0,.65)', padding:18 }}>
        ...
        <div style={{ border:'1px dashed rgba(255,255,255,.22)', borderRadius:20, padding:14, background:'rgba(255,255,255,.04)', textAlign:'center' }}>
          {preview
            ? <img src={preview} alt="foto" style={{ width:'100%', maxHeight:260, objectFit:'contain', borderRadius:16 }} />
            : <div style={{ minHeight:150, display:'flex', alignItems:'center', justifyContent:'center', color:'rgba(255,255,255,.62)', fontWeight:900, padding:'0 8px' }}>Scatta una foto o caricala dalla galleria</div>}
          <div style={{ display:'grid', gridTemplateColumns:'repeat(2,minmax(0,1fr))', gap:8, marginTop:12 }}>
            <button type="button" onClick={() => cameraInputRef.current?.click()} style={{ minHeight:46, borderRadius:14, border:'1px solid rgba(255,255,255,.14)', background:'rgba(255,255,255,.07)', color:'white', fontFamily:'inherit', fontSize:12.5, fontWeight:1000, cursor:'pointer' }}>📷 Scatta foto</button>
            <button type="button" onClick={() => galleryInputRef.current?.click()} style={{ minHeight:46, borderRadius:14, border:'1px solid rgba(255,255,255,.14)', background:'rgba(255,255,255,.07)', color:'white', fontFamily:'inherit', fontSize:12.5, fontWeight:1000, cursor:'pointer' }}>🖼 Carica foto</button>
          </div>
          <input ref={cameraInputRef} type="file" accept="image/*" capture="environment" onChange={handleInputChange} style={{ display:'none' }} />
          <input ref={galleryInputRef} type="file" accept="image/*" onChange={handleInputChange} style={{ display:'none' }} />
        </div>
        ...
        {aiResult && <div style={{ marginTop:16, borderRadius:18, background:'rgba(255,255,255,.055)', border:`1px solid ${hexToRgba(aiTone,.35)}`, padding:13 }}>
          ...
          <div style={{ color:'rgba(255,255,255,.76)', fontSize:12, lineHeight:1.42, marginTop:6 }}>
            {aiStatus === 'unusable'
              ? 'Non riesco a identificare l\'animale in questa foto.'
              : aiStatus === 'alternatives'
                ? 'Ho più ipotesi plausibili. Scegli quella che ti sembra corretta.'
                : (activeInDex
                  ? (aiResult.summary || `Sembra ${activeMatch.animal.com || activeMatch.animal.sci}.`)
                  : (aiResult.summary || `Sembra ${activeMatch?.common_name || 'una specie non ancora presente nel Dex'}.`))}
          </div>
          {aiResult.image_quality && aiStatus !== 'unusable' && <div style={{ color:'rgba(255,255,255,.42)', fontSize:10.5, marginTop:6 }}>Qualità foto: {aiResult.image_quality}</div>}
          {aiStatus === 'unusable' && (
            <div style={{ marginTop:10, borderRadius:14, background:'rgba(0,0,0,.22)', padding:'10px 12px' }}>
              <div style={{ color:'rgba(255,255,255,.58)', fontSize:10.5, fontWeight:1000, textTransform:'uppercase', letterSpacing:.6, marginBottom:6 }}>Suggerimenti rapidi</div>
              <ul style={{ margin:0, paddingLeft:18, color:'rgba(255,255,255,.72)', fontSize:11.5, lineHeight:1.45 }}>
                {PHOTO_ID_TIPS.map(tip => <li key={tip} style={{ marginBottom:4 }}>{tip}</li>)}
              </ul>
              {(aiResult.unusable_reason || aiResult.summary) && <div style={{ color:'rgba(255,255,255,.48)', fontSize:11, marginTop:8, lineHeight:1.35 }}>{aiResult.unusable_reason || aiResult.summary}</div>}
            </div>
          )}
        </div>}
        {aiStatus === 'alternatives' && !!matches.length && (
          <div style={{ marginTop:12 }}>
            {matches.map(match => (
              <RecognitionAlternativeRow
                key={match.key}
                match={match}
                selected={selectedMatch?.key === match.key}
                onSelect={(picked) => setSelectedMatch(picked)}
                onRequestAdd={submitAddRequest}
                requestBusy={requestBusy}
              />
            ))}
          </div>
        )}
        {activeMatch && (aiStatus === 'certain' || selectedMatch) && (
          <div style={{ marginTop:12, borderRadius:18, border:'1px solid rgba(255,255,255,.10)', background:'rgba(255,255,255,.045)', padding:12 }}>
            {activeInDex ? (
              <>
                <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:12 }}>
                  ...
                  <div style={{ minWidth:0 }}>
                    ...
                    <div style={{ color:'#90D84A', fontSize:11, fontWeight:1000, marginTop:6 }}>{Math.round(activeMatch.probability || 0)}% confidenza</div>
                  </div>
                </div>
              ...
              <>
                <div style={{ color:'white', fontSize:14, fontWeight:1000, lineHeight:1.15 }}>{activeMatch.common_name || 'Specie non nel Dex'}</div>
                {activeMatch.scientific_name && <div style={{ color:'rgba(255,255,255,.56)', fontSize:11.5, marginTop:4 }}>{activeMatch.scientific_name}</div>}
                <div style={{ color:'rgba(255,255,255,.62)', fontSize:12, lineHeight:1.4, marginTop:8 }}>Questa specie non è ancora nel Dex. Puoi chiedere di aggiungerla.</div>
                <button type="button" disabled={requestBusy} onClick={() => submitAddRequest(activeMatch)} style={{ marginTop:12, width:'100%', minHeight:44, borderRadius:14, border:'1px solid rgba(240,168,64,.34)', background:'rgba(240,168,64,.12)', color:'#FFD9A8', fontFamily:'inherit', fontSize:12.5, fontWeight:1000, cursor:requestBusy ? 'default' : 'pointer' }}>
                  {requestBusy ? 'Invio richiesta…' : 'Richiedi aggiunta animale'}
                </button>
              </>
            )}
          </div>
...
function CountryVisitPromptModal({ animal, visitedCountries = [], onClose, onConfirm }) {
  const [search, setSearch] = useState('');
  const [selected, setSelected] = useState([]);
  const countryCodes = useMemo(() => {
    const visited = new Set(normalizeIsoList(visitedCountries));
    return getAnimalCountryCodes(animal)
      .filter(code => !visited.has(code))
      .sort((a,b)=>getCountryDisplayName(a).localeCompare(getCountryDisplayName(b), 'it'));
  }, [animal?.id, JSON.stringify(visitedCountries || [])]);
  ...
  const toggle = (code) => {
    ...
    setSelected(prev => prev.includes(iso) ? prev.filter(item => item !== iso) : [...prev, iso]);
  };
  if (!animal || !countryCodes.length) return null;
  return (
    <div onClick={onClose} style={{ position:'fixed', inset:0, zIndex:270, background:'rgba(0,0,0,.78)', display:'flex', alignItems:'flex-end', justifyContent:'center', padding:'calc(env(safe-area-inset-top, 0px) + 12px) 12px calc(env(safe-area-inset-bottom, 0px) + 12px)' }}>
      <div onClick={e=>e.stopPropagation()} data-animaldex-modal="true" style={{ width:'100%', maxWidth:460, maxHeight:'min(82dvh, 680px)', overflow:'hidden', borderRadius:28, background:'linear-gradient(180deg,#202126,#111113)', border:'1px solid rgba(144,216,74,.24)', boxShadow:'0 30px 80px rgba(0,0,0,.62)', color:'white', display:'flex', flexDirection:'column' }}>
        <div style={{ padding:18, borderBottom:'1px solid rgba(255,255,255,.08)', flexShrink:0 }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:12 }}>
            <div style={{ minWidth:0 }}>
              <div style={{ color:'#90D84A', fontSize:11, fontWeight:1000, textTransform:'uppercase', letterSpacing:.8 }}>Paesi compatibili</div>
              <div style={{ fontSize:22, fontWeight:1000, lineHeight:1.08, marginTop:5 }}>Hai visitato qualcuna di queste nazioni?</div>
              <div style={{ color:'rgba(255,255,255,.62)', fontSize:12.5, lineHeight:1.45, marginTop:8 }}>{animal.com || animal.sci} risulta presente in questi paesi, ma nessuno è ancora nella tua scratch map.</div>
            </div>
            <button onClick={onClose} aria-label="Chiudi" style={{ width:38, height:38, borderRadius:14, border:'1px solid rgba(255,255,255,.10)', background:'rgba(255,255,255,.07)', color:'white', fontSize:22, flexShrink:0 }}>×</button>
          </div>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cerca nazione o ISO..." style={{ marginTop:14, width:'100%', height:44, borderRadius:16, background:'#252527', color:'white', border:'1px solid rgba(255,255,255,.12)', padding:'0 13px', fontSize:14, boxSizing:'border-box', outline:'none' }} />
        </div>
        <div style={{ flex:1, minHeight:0, overflowY:'auto', WebkitOverflowScrolling:'touch', display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, padding:14 }}>
          {filtered.map(code => {
            const active = selected.includes(code);
            return (
              <button key={code} onClick={()=>toggle(code)} style={{ minHeight:50, borderRadius:17, border:`1px solid ${active?'rgba(144,216,74,.72)':'rgba(255,255,255,.09)'}`, background:active?'rgba(144,216,74,.18)':'rgba(255,255,255,.045)', color:active?'#D8FFC4':'white', display:'flex', alignItems:'center', gap:8, padding:'8px 10px', cursor:'pointer', textAlign:'left', fontFamily:'inherit' }}>
                <span style={{ fontSize:22 }}>{getFlagEmoji(code)}</span>
                <span style={{ flex:1, minWidth:0, fontSize:11.5, fontWeight:950, lineHeight:1.15 }}>{getCountryDisplayName(code)}</span>
                <span style={{ color:active?'#90D84A':'rgba(255,255,255,.24)', fontWeight:1000 }}>{active?'✓':'+'}</span>
              </button>
            );
          })}
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1.5fr', gap:8, padding:'10px 14px 16px', borderTop:'1px solid rgba(255,255,255,.08)', flexShrink:0 }}>
          <button onClick={onClose} style={{ minHeight:46, borderRadius:16, border:'1px solid rgba(255,255,255,.10)', background:'rgba(255,255,255,.06)', color:'rgba(255,255,255,.72)', fontWeight:1000, fontFamily:'inherit' }}>Non ora</button>
          <button disabled={!selected.length} onClick={()=>onConfirm?.(selected)} style={{ minHeight:46, borderRadius:16, border:'none', background:selected.length?'#90D84A':'#3A3A3C', color:selected.length?'#111':'rgba(255,255,255,.38)', fontWeight:1000, fontFamily:'inherit' }}>{selected.length ? `Aggiungi ${selected.length}` : 'Seleziona paesi'}</button>
        </div>
      </div>
    </div>
  );
...
<div style={{ display:'flex', flexWrap:'wrap', gap:3, alignItems:'center', marginBottom:14 }}>
  {[
    ...
    <span key={i} style={{ display:'flex', alignItems:'center', gap:3 }}>
      <button onClick={()=>onOpenTaxonomyFilter?.({ key, value:p, animal:a })} style={{ color:i===2 ? c.accent : 'rgba(255,255,255,.42)', fontSize:11, fontWeight:i===2?700:500, fontStyle:i===2?'normal':'italic', background:'transparent', border:'none', padding:0, cursor:'pointer', textDecoration:'underline', textDecorationThickness:i===2 ? 1.5 : 1, textUnderlineOffset:2 }}>{p}</button>
              ...
              {statMode==='tassonomia'&&(
                <div data-animaldex-card="true" style={{ background:'rgba(0,0,0,.35)', borderRadius:24, padding:'4px 16px 16px', height:'100%', boxSizing:'border-box', display:'flex', flexDirection:'column', justifyContent:'space-between' }}>
                  {[
                    ...
                    <button key={l} disabled={!key || !v} onClick={()=>key && v && onOpenTaxonomyFilter?.({ key, value:v, animal:a })} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:10, padding:'10px 0', border:'none', borderBottom:'1px solid rgba(255,255,255,.06)', background:'transparent', fontFamily:'inherit', cursor:key && v ? 'pointer' : 'default', textAlign:'left' }}>
                      <span style={{ color:'rgba(255,255,255,.35)', fontSize:12 }}>{l}</span>
                      <span style={{ color:l==='Specie'||l==='Genere'?c.accent:'white', fontSize:12, fontWeight:700, fontStyle:l==='Specie'||l==='Genere'?'italic':undefined, textDecoration:key?'underline':'none', textUnderlineOffset:3, maxWidth:'62%', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{v}</span>
                    </button>
                  ))}
                </div>
              )}
...
function getFeatureBioregionId(feature) {
  const p = feature?.properties || {};
  const id = String(p.bioregion_id || p.unit_id || p.id || p.ID || '');
  if (/^MAR_R_002_(ATL|MED|BLK)$/i.test(id)) return 'MAR_R_002';
  return id;
}
  ...
  const recenter = () => {
    ...
    if (fitCenter && Number.isFinite(fitCenter[0]) && Number.isFinite(fitCenter[1])) {
      map.easeTo({
        ...
        zoom:Number.isFinite(fitZoom) ? fitZoom : zoomForLngLatBounds(target, fullscreen),
...
function BioregionVectorMap({ highlightIds = [], focusIds = [], highlightIsoCodes = [], selectedId=null, onSelect, clickable=false, height=180, accent='#A84637', marine=false, domain='auto', showLabels=false, fullBleed=false, fullscreen=false, onCloseFullscreen, fullscreenSelectionResolver=null, showFeatureBoundaries=true, levelGroups=[], interactive=true, hideInactiveFill=true, layerOpacityScale=1, autoSpin=false, cameraId=null, fitDuration=null }) {
  ...
  const focusSet = new Set((focusIds || []).map(String).filter(Boolean));
  const focusFeatures = focusSet.size ? coloredRelevant.filter(f => focusSet.has(String(getFeatureBioregionId(f) || ''))) : [];
  ...
  const cameraPreset = TERRITORY_CAMERA_PRESETS[String(cameraId || selectedId || '')] || null;
  const mapLibreCenter = cameraPreset?.center || (focusFeatures.length ? mergeLngLatCenters(focusOrActiveFeatures) : (mapLibreBounds && !boundsAreGlobalEnoughForSpin(mapLibreBounds) ? mergeLngLatCenters(focusOrActiveFeatures) : null));
...
function lifeNodeToTaxFilter(node) {
  if (!node) return null;
  const rankToKey = { kingdom:'kin', phylum:'phy', class:'cls', order:'ord', family:'fam', genus:'genus' };
  const direct = node.match && Object.keys(node.match).length === 1 ? Object.entries(node.match)[0] : null;
  if (direct) return { key:direct[0], value:direct[1] };
  const key = rankToKey[node.rank];
  if (!key || node.kind === 'animal') return null;
      ...
      {detailNode && (
        <div onClick={()=>setDetailNode(null)} style={{ position:'absolute', inset:0, zIndex:42, background:'rgba(0,0,0,.70)', display:'flex', alignItems:'center', justifyContent:'center', padding:18 }}>
          <div onClick={e=>e.stopPropagation()} style={{ width:'100%', maxWidth:420, borderRadius:26, background:'linear-gradient(180deg,#1A1D21,#0F1114)', border:`1px solid ${detailNode.node.color}88`, boxShadow:'0 26px 80px rgba(0,0,0,.60)', padding:18, color:'white' }}>
            ...
            {(() => {
              const filter = lifeNodeToTaxFilter(detailNode.node);
              if (!filter?.key || !filter?.value) return null;
              return <button onClick={()=>{ setDetailNode(null); onOpenTaxonomyFilter?.({ ...filter, animal:null }); }} style={{ marginTop:14, width:'100%', minHeight:46, borderRadius:16, border:'none', background:'#244A70', color:'white', fontWeight:1000, fontFamily:'inherit', cursor:'pointer' }}>Vedi animali di questo nodo</button>;
            })()}
          </div>
        </div>
      )}
...
const lifeZoomButtonStyle = { width:40, height:40, borderRadius:15, border:'1px solid rgba(255,255,255,.12)', background:'rgba(13,15,18,.82)', color:'white', fontSize:18, fontWeight:1000, boxShadow:'0 12px 26px rgba(0,0,0,.28)' };
...
function LifePathRail({ path, onOpen, isLight }) {
  ...
  return <div ref={railRef} style={{ height:34, display:'flex', alignItems:'center', gap:7, overflowX:'auto', overflowY:'hidden', WebkitOverflowScrolling:'touch', scrollbarWidth:'none', paddingRight:2 }}>
    {path.map((node, index) => {
...
function TaxonomyExplorer({ animals=ANIMALS, statusMap={}, visitedCountries=[], initialAnimal=null, onBack, onOpenAnimal, onOpenTaxonomyFilter, theme='dark' }) {
...
function FriendsPage({ onBack, user, userProfile, accessToken = '', initialTab = 'feed', onSocialChange, statusMap = {}, visitedCountries = [], earnedBadgeIds = [], theme='dark' }) {
  ...
  const getSearchRelationLabel = (profileId) => {
    ...
    if (pendingOutIdSet.has(profileId)) return 'In attesa';
    if (pendingInIdSet.has(profileId)) return 'Richiesta ricevuta';
    return 'Aggiungi';
  };
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        ...
        if (alive && !controller?.signal?.aborted) setSearching(false);
      }
    }, 320);
...
function RegionsPage({ onBack, statusMap = {}, visitedCountries = [], onVisitedCountriesChange, onRemoveDestination, initialView, onSelect, onOpenCountry, onOpenRegion, onAddDestination, destinationsLoading=false, onOpenHabitatGrid, theme='dark', animalsWithStatus: animalsWithStatusProp = null }) {
  ...
  useEffect(() => {
    ...
    if (!['ecoregions','habitats','lifeweb'].includes(view)) setSelectedEcoregion(null);
  ...
  const scrollToMainMap = () => {
    requestAnimationFrame(() => {
      mainRegionMapRef.current?.scrollIntoView({ behavior:'smooth', block:'start' });
    });
  };
...
function SimpleComparatorPage({ onBack, animals = [], statusMap = {}, visitedCountries = [], initialAnimal = null, theme='dark' }) {
  const isLightTheme = theme === 'light';
  ...
  const panelBg = isLightTheme ? 'rgba(255,255,255,.72)' : 'rgba(255,255,255,.055)';
  ...
  const firstDefault = useMemo(() => {
    if (initialAnimal?.id) return normalized.find(a => String(a.id) === String(initialAnimal.id)) || initialAnimal;
  ...
  useEffect(() => {
    setLeftId(firstDefault?.id ? String(firstDefault.id) : '');
    setRightId(secondDefault?.id ? String(secondDefault.id) : '');
  }, [firstDefault?.id, secondDefault?.id]);
  ...
  const statValue = (animal, key) => {
    ...
    if (key === 'speed') return safeNumber(animal?.stats?.velocita, ' km/h');
  ...
  const AnimalPanel = ({ animal, accent, side }) => {
    ...
    return (
      <div style={{ borderRadius:24, border:`1px solid ${hexToRgba(accent,.42)}`, background:isLightTheme?'rgba(255,255,255,.72)':'linear-gradient(180deg,rgba(255,255,255,.07),rgba(255,255,255,.035))', padding:14, boxShadow:`0 16px 38px ${hexToRgba(accent,.10)}` }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          <div style={{ width:86, height:86, borderRadius:22, background:cls.img || '#202228', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden', flex:'0 0 auto' }}>
            <AnimalImg a={{...animal, status:'avvistato'}} size={82} fontSize={40} overrideStatus="avvistato" />
          </div>
          <div style={{ minWidth:0, flex:1 }}>
            <div style={{ color:accent, fontSize:11, fontWeight:1000, textTransform:'uppercase', letterSpacing:.8 }}>{side}</div>
            <div style={{ color:pageText, fontSize:20, fontWeight:1000, lineHeight:1.08, marginTop:3 }}>{animal.com}</div>
            <div style={{ color:subText, fontSize:11.5, fontStyle:'italic', marginTop:4, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{animal.sci}</div>
          </div>
        </div>
  ...
  return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', background:pageBg, overflow:'hidden' }}>
      ...
      <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', padding:'14px 14px calc(30px + env(safe-area-inset-bottom, 0px))', boxSizing:'border-box' }}>
        <div style={{ borderRadius:24, border:`1px solid ${panelBorder}`, background:isLightTheme?'rgba(255,255,255,.66)':'linear-gradient(135deg,rgba(168,70,55,.24),rgba(20,20,22,.88))', padding:16, marginBottom:14 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr', gap:12, marginBottom:12 }}>
          ...
          <AnimalPanel animal={right} accent={COMPARE_RIGHT_COLOR} side="Animale B" />
        </div>
        <div style={{ borderRadius:24, border:`1px solid ${panelBorder}`, background:panelBg, overflow:'hidden' }}>
          {rows.map(([label,key], index) => (
            <div key={key} style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, alignItems:'center', padding:'12px 13px', borderTop:index ? `1px solid ${panelBorder}` : 'none' }}>
              <div style={{ color:subText, fontSize:11.5, fontWeight:950, textTransform:'uppercase', letterSpacing:.5 }}>{label}</div>
              <div style={{ color:COMPARE_LEFT_COLOR, fontSize:13, fontWeight:1000, textAlign:'center', minWidth:0, overflow:'hidden', textOverflow:'ellipsis' }}>{statValue(left, key)}</div>
              <div style={{ color:COMPARE_RIGHT_COLOR, fontSize:13, fontWeight:1000, textAlign:'right', minWidth:0, overflow:'hidden', textOverflow:'ellipsis' }}>{statValue(right, key)}</div>
            </div>
          ))}
        </div>
      </div>
...
function getSeenSectionIntros() {
  try {
    const next = new Set(sectionIntroSeenMemory);
    JSON.parse(window.localStorage.getItem(SECTION_INTRO_SEEN_KEY) || '[]').forEach(section => next.add(section));
    JSON.parse(window.sessionStorage.getItem(SECTION_INTRO_SEEN_SESSION_KEY) || '[]').forEach(section => next.add(section));
    ['regions', 'badges', 'abilities', 'compare', 'profile', 'gallery', 'lifeweb', 'quickSeen', 'friends', 'taxonomy'].forEach(section => {
      ...
      if (window.sessionStorage.getItem(`apex_section_intro_${section}`)) next.add(section);
      if (document.cookie.includes(`apex_section_intro_${section}=1`)) next.add(section);
    });
    sectionIntroSeenMemory = next;
    return next;
  } catch {
    return new Set(sectionIntroSeenMemory);
  }
}
...
function markSectionIntroSeen(section) {
  ...
  try {
    ...
    seen.add(section);
    sectionIntroSeenMemory = seen;
    window.localStorage.setItem(SECTION_INTRO_SEEN_KEY, JSON.stringify(Array.from(seen)));
    window.sessionStorage.setItem(SECTION_INTRO_SEEN_SESSION_KEY, JSON.stringify(Array.from(seen)));
    window.localStorage.setItem(`apex_section_intro_${section}`, '1');
    window.sessionStorage.setItem(`apex_section_intro_${section}`, '1');
    document.cookie = `apex_section_intro_${section}=1; Max-Age=31536000; Path=/; SameSite=Lax`;
  } catch {}
}
...
@media (max-width: 560px), (display-mode: standalone) {
  ...
  const handleStatusChange = async (id, status) => {
    ...
    const nextCountryCodes = getAnimalCountryCodes(currentAnimal);
    const visitedSet = new Set(normalizeIsoList(visitedCountries));
  ...
  const handleVisitedCountriesChange = (nextList = []) => {
    const clean = normalizeIsoList(nextList);
    setVisitedCountries(clean);
    saveVisitedCountries(clean);
  };

  const handleRemoveDestination = async (iso) => {
    ...
    try {
      ...
      await trackUserEvent(user, 'country_removed', { country_iso:cleanIso, source_screen:'regions' }, userProfile);
    } catch (err) {
      console.warn('[Apex] Rimozione destinazione fallita:', err);
      setDataError(err?.message || 'Errore rimozione paese');
    }
  };

  const confirmPromptCountries = async (codes = []) => {
  ...
  const handleCompleteOnboarding = async ({ nickname, countries, seenAnimalIds, tripTags, demographics = {} }) => {
    ...
    try {
      ...
      const cleanCountries = normalizeIsoList(countries || []);
...
const openPage = (nextPage, returnPage = 'menu') => {
  const apply = () => {
  setSel(null);
  setPhotoTarget(null);
  setGridReturnTarget(null);
  ...
  setSectionReturnPage(returnPage);
  ...
  try {
    if (user?.id) {
      await supabase.from('animal_photos').insert({
        ...
        recognition_status:meta.ai_status || 'confirmed',
...
const requestAnimalAdd = async (payload = {}) => {
  const commonName = String(payload.common_name || '').trim();
  if (!commonName) return { ok:false };
  const body = {
    user_id:user?.id || null,
    user_email:user?.email || userProfile?.email || null,
    common_name:commonName,
    scientific_name:payload.scientific_name || '',
    photo_url:payload.photo_url || '',
    photo_storage_path:payload.photo_storage_path || '',
    ai_summary:payload.ai_summary || '',
    ai_probability:payload.ai_probability ?? null,
    lat:payload.gps?.lat ?? null,
    lng:payload.gps?.lng ?? null,
  };
  try {
    const response = await fetch('/api/request-animal-add', {
      method:'POST',
      headers:{ 'Content-Type':'application/json' },
    ...
    if (response.ok && data.stored) {
      await trackUserEvent(user, 'animal_add_requested', { common_name:commonName, scientific_name:body.scientific_name || null, stored:true, notified:!!data.notified }, userProfile);
      return { ok:true, mode:'database', notified:!!data.notified };
    }
    if (response.ok && data.fallback === 'mailto') {
      const adminEmail = process.env.REACT_APP_APEX_ADMIN_EMAIL || '';
      if (adminEmail && typeof window !== 'undefined') {
        const subject = encodeURIComponent(`Richiesta animale Apex: ${commonName}`);
        const text = encodeURIComponent([
          `Nome comune: ${commonName}`,
          `Nome scientifico: ${body.scientific_name || '-'}`,
          `Utente: ${body.user_email || user?.id || '-'}`,
          `Foto: ${body.photo_url || '-'}`,
          `Note AI: ${body.ai_summary || '-'}`,
        ].join('\n'));
        window.location.href = `mailto:${adminEmail}?subject=${subject}&body=${text}`;
        return { ok:true, mode:'mailto' };
      }
    }
    if (!response.ok) throw new Error(data?.error || 'Richiesta non salvata.');
  } catch (err) {
    console.warn('[Apex] request-animal-add api:', err);
  }
  if (user?.id) {
    try {
      const { error } = await supabase.from('animal_add_requests').insert({ ...body, user_id:user.id, status:'pending' });
      if (!error) {
        await trackUserEvent(user, 'animal_add_requested', { common_name:commonName, scientific_name:body.scientific_name || null, stored:true, notified:false }, userProfile);
        return { ok:true, mode:'supabase' };
      }
    } catch (err) {
      console.warn('[Apex] animal_add_requests insert:', err);
    }
  }
  const adminEmail = process.env.REACT_APP_APEX_ADMIN_EMAIL || '';
  if (adminEmail && typeof window !== 'undefined') {
...
const openTaxonomyFilterFromDetail = ({ key, value, animal }) => {
  const labels = { kin:'Regno', phy:'Phylum', cls:'Classe', ord:'Ordine', fam:'Famiglia', gen:'Genere' };
  if (!key || !value) return;
  setGridReturnTarget(animal ? { page:'detail', animal } : { page:'taxonomy' });
  setSel(null);
  setGridPreset({
    id: Date.now(),
    type:'taxonomy-node',
...
const returnFromSection = (fallback = 'menu') => {
  const target = sectionReturnPage || fallback;
  setSectionReturnPage('menu');
  setPage(target);
};
