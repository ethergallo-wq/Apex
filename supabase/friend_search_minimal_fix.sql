-- Minimal Animaldex friend search fix.
-- Run this in Supabase SQL Editor on project tbprygwksuayakhhtkpq.
--
-- Scope: only what the app needs to search users:
-- - public.user_profiles readable by authenticated users, writable only by owner
-- - public.user_blocks respected by search
-- - public.search_social_profiles(text, integer) executable by authenticated users
...
select
  'authenticated can execute search rpc' as check_name,
  has_function_privilege('authenticated', 'public.search_social_profiles(text, integer)', 'EXECUTE') as ok
union all
select
  'authenticated can select user_profiles',
  has_table_privilege('authenticated', 'public.user_profiles', 'SELECT')
union all
select
  'authenticated can update own profile table',
  has_table_privilege('authenticated', 'public.user_profiles', 'UPDATE')
union all
select
  'authenticated can read own blocks',
